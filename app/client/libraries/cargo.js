...
Template.cargo.events({
  'click #pay_seller': function() {
    ...
    Meteor.call('paid', this._id, function(error, success) {
  ...
  'click #cargo_shipped': function() {
    $('#cargo_shipped').replaceWith('<div class="form-group"><h2>Date of Arrival</h2><input id="dateofarrival" type="date" class="form-control inputArrival"><button id="submitDate" class="btn btn-lg btn-success shipment_button">Ask for Payment</button></div>');
  },
  'click #submitDate': function() {
    $('#shipment').html('<div class="cargo_header shipment_header"><h2>Cargo arrived at Port</h2><img src="/cat_sleeping.jpg" /><button id="disputePayment" class="btn btn-lg btn-danger shipment_button">Dispute Payment</button></div>');
  },
